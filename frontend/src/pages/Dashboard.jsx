...
  useEffect(() => {
    if (!bounds) return;
    map.flyToBounds(bounds, { padding: [30, 30], maxZoom: 9, animate: true, duration: 0.8 });
  }, [bounds, map]);
  return null;
};
...
      mapRef.current.flyTo(INITIAL_CENTER, INITIAL_ZOOM, { animate: true, duration: 0.8 });
    }
  };
...
      <div style={{ display: "flex", gap: 22, padding: 22, height: "calc(100vh - 120px)" }}>
        
        {/* MAP - Retaining original visual style */}
        <div style={{ flex: 2, borderRadius: 10, overflow: "hidden", border: `1px solid ${BORDER_GREY}`, background: "#fff" }}>
          <MapContainer
...
          }}
        >
          <div style={{ fontWeight: 800, color: GOV_NAVY, marginBottom: 15, fontSize: "16px" }}>
            Heatmap Legend
          </div>
...
              <div style={{ width: 24, height: 14, background: l.c, borderRadius: 3 }} />
              <div style={{ fontSize: 13, color: "#475569", fontWeight: 500 }}>{l.t}</div>
            </div>
          ))}
...
        {loading && (
          <div style={{ background: "#f1f5f9", padding: "6px 12px", borderRadius: 20, fontSize: 11, border: "1px solid #e2e8f0" }}>
            Fetching Live Data...
          </div>
        )}
        {error && (
          <div style={{ background: "#fef2f2", color: "#b91c1c", padding: "6px 12px", borderRadius: 20, fontSize: 11, border: "1px solid #fee2e2" }}>
            {error}
          </div>
        )}
      </div>
